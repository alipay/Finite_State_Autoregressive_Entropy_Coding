# from McQuic (https://github.com/xiaosu-zhu/McQuic)
...
class AlignedCrop(nn.Module):
    def __init__(self, base: int = 128, max_size: Optional[Tuple[int, int]] = None):
        super().__init__()
        self._base = base
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.max_size is not None:
            wCrop = max(wCrop, w - self.max_size[0])
            hCrop = max(hCrop, h - self.max_size[1])
        cropLeft = wCrop // 2
...
class AlignedPadding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.ndim == 3:
            x = x.unsqueeze(0)
            has_batch_dim = False
        x = F.pad(x, (padLeft, padTop, padRight, padBottom), padding_mode="reflect")
        if not has_batch_dim:
            x = x.squeeze(0)
