# https://github.com/sony/sqvae/blob/main/vision/quantizer.py
import torch
...
def sample_gumbel(logits, eps=1e-10):
    U = torch.rand(logits.shape).type_as(logits)
    ...
    def _quantize(self, z_from_encoder, var_q, codebook, flg_train=True, flg_quant_det=False):
        ...
        if flg_train:
            ...
            if flg_quant_det:
                ...
                encodings_hard = torch.zeros(indices.shape[0], self.size_dict).type_as(logit)
