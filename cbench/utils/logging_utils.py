...
class SmoothedValue:
    ...
    @property
    def median(self):
        if len(self.deque) == 0: return 0 # avoid empty tensor median error
...
class TimeProfiler(object):
    def __init__(self, meter: SmoothedValue) -> None:
        self.meter = meter

    @staticmethod
    def start_profile(meter):
        return TimeProfiler(meter)
...
class MetricLogger:
    def __init__(self, delimiter="\t", meter_default_config=dict()):
    ...
    def get_global_average(self):
        ...
        for name, meter in self.meters.items():
            metric_dict[name] = meter.global_avg
        return metric_dict

    def start_time_profile(self, name):
        # TimeProfiler logs time in milliseconds!
        if not name in self.meters:
            name = name + " (ms)"
            self.add_meter(name, self.default_meter_constructor())
        return TimeProfiler(self.meters[name])

    # TODO: this cause infinite loop when using multiprocessing. Wondering why...
...
def reduce_across_processes(val):
    if not is_dist_avail_and_initialized() or not torch.cuda.is_available():
