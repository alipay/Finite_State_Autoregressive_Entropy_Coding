...
import configs.presets.exp_abl_model_list_shallow as default_models
import configs.presets.exp_abl_vq_model_list_shallow as default_models_vq

gpu_available = torch.cuda.is_available()
num_gpus = torch.cuda.device_count()
batch_size_total = 1000
...
config = GroupedCodecBenchmarkBuilder(
    codec_group_builder = import_config_from_module(default_models) + import_config_from_module(default_models_vq),
    benchmark_builder=
        import_config_from_module(default_benchmark)
        .set_override_name("model-shallow-cifar10-exp-abl")
        .update_slot_params(
